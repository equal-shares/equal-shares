...
def bids_from_approvals(
    approval_ballots: dict[int, set[int]], projects_costs: dict[int, float]
    ...
    """
    result: defaultdict[int, dict[int, float]] = defaultdict(dict)
...
def bids_from_reverse_approvals(
    reverse_approval_ballots: dict[int, list[int]], projects_costs: dict[int, int]
) -> dict[int, dict[int, int]]:
    """
    Given approval votes and project costs, creates bids assuming the minimum of each project equals its maximum.

    >>> bids_from_reverse_approvals({
    ...     11: {1, 2, 3},
    ...     12: {1},
    ...     13: {1, 2},
    ...     14: {3},
...
def example1() -> None:
    ...
    winners_allocation2 = equal_shares_original(voters, list(projects_costs.keys()), projects_costs, approvals, budget)
    print("Original MES: ", winners_allocation2)
