...
import logging, numpy as np, copy
...
def average_first(
    ...
    >>> winners_allocations, candidates_payments_per_voter = average_first(voters, cost_min_max, 900, bids, use_plt=False)
    ...
    for project_id, min_cost in projects_min_costs.items():
        if averages[project_id] >= min_cost:
            allocation = averages[project_id]
            winners_allocations[project_id] = allocation
            projects_min_costs[project_id] = 0

            # Update bids:
            bids_not_zero[project_id] = {
                voter: max(0, bid-allocation)
                for voter,bid in bids_not_zero[project_id].items()
            }
            budget -= allocation
        else:
            winners_allocations[project_id] = 0
            projects_min_costs[project_id] = min_cost
    winners_additional_allocations, candidates_payments_per_voter = equal_shares(voters, projects_min_costs, budget, bids_not_zero)
